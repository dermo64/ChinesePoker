import { compareFive, compareLex } from './rules.js';
function compareFront(a, b) {
    if (a.size !== 3 || b.size !== 3)
        throw new Error('compareFront expects (3,3)');
...
export function scoreRound(input) {
    ...
    if (user.foul) {
        return {
            ...
            scoop: true,
            total: -6,
            explanation: 'Foul: your hands violate Back >= Middle >= Front. Automatic loss (-6).'
        };
    }
    ...
    if (userWinsAll)
        explanationParts.push('Scoop bonus: +3');
    if (userLosesAll)
        explanationParts.push('Scoop penalty: -3');
    return {
        perHand,
        scoop,
        total,

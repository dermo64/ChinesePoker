...
function isStraight(ranksDesc: number[]) {
  const uniq = Array.from(new Set(ranksDesc)).sort((a, b) => b - a);
  if (uniq.length !== 5) return { ok: false as const, high: 0 };
  ...
  if (max === 14 && uniq[1] === 5 && uniq[2] === 4 && uniq[3] === 3 && uniq[4] === 2) {
    return { ok: true as const, high: 5 };
  }

  if (max - min !== 4) return { ok: false as const, high: 0 };
  for (let i = 0; i < 4; i++) {
    if (uniq[i] - 1 !== uniq[i + 1]) return { ok: false as const, high: 0 };
  }
  return { ok: true as const, high: max };
}
...
export function evaluate3(cards: string[]): HandValue {
...
export function describeHandValue(v: HandValue): string {
  if (v.size === 3) {
    if (v.category === 2) return `Three of a kind (${rankLabel(v.tiebreak[0])})`;
    if (v.category === 1) return `One pair (${rankLabel(v.tiebreak[0])})`;
    return `High card (${rankLabel(v.tiebreak[0])})`;
  }

  switch (v.category) {
    case 8:
      return `Straight flush (${rankLabel(v.tiebreak[0])})`;
    case 7:
      return `Four of a kind (${rankLabel(v.tiebreak[0])})`;
    case 6:
      return `Full house (${rankLabel(v.tiebreak[0])} over ${rankLabel(v.tiebreak[1])})`;
    case 5:
      return `Flush (${rankLabel(v.tiebreak[0])})`;
    case 4:
      return `Straight (${rankLabel(v.tiebreak[0])})`;
    case 3:
      return `Three of a kind (${rankLabel(v.tiebreak[0])})`;
    case 2:
      return `Two pair (${rankLabel(v.tiebreak[0])} & ${rankLabel(v.tiebreak[1])})`;
    case 1:
      return `One pair (${rankLabel(v.tiebreak[0])})`;
    default:
      return `High card (${rankLabel(v.tiebreak[0])})`;
  }
}
